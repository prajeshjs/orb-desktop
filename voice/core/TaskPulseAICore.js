 ...
 * TaskPulse AI Core
 * 
 * The central brain of TaskPulse. It bridges the Natural Language Understanding (Planner)
...
class TaskPulseAICore {
     ...
     */
    init(appAdapter) {
        console.log('[TaskPulseAICore] Initializing Intelligence Layer...');
    ...
    async processRequest(rawInput, session) {
        if (!this.isInitialized) throw new Error("TaskPulse AI Core not initialized");
        ...
        try {
            ...
            console.log(`[TaskPulseAICore] Processed request in ${Date.now() - startTime}ms`);
            
            return response;
            
        } catch (error) {
            console.error('[TaskPulseAICore] Error processing request:', error);
            return { success: false, message: "System error: " + error.message };
        }
    }
}

module.exports = new TaskPulseAICore();
